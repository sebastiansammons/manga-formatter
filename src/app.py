...
def settings():
    if request.method == "GET":
        ...
        return render_template(
            ...
            active_len=len(active_manga),
            inactive_manga=inactive_manga,
            inactive_len=len(inactive_manga),
        )
    else:
...
def auto_format():
    if request.method == "GET":
        ...
        return render_template(
            "auto_format.html",
            manga_chapter_title=manga_chapter_title,
            manga_chapter_status=manga_chapter_status,
            manga_volume_title=manga_volume_title,
            manga_volume_status=manga_volume_status,
            len=len(manga_chapter_status),
        )
    else:
...
def auto_chapter():
    if request.method == "GET":
        active_manga = manga.get_active_manga()
        return render_template(
            "auto_chapter.html", active_manga=active_manga, len=len(active_manga)
        )
    else:
...
def auto_volume():
    if request.method == "GET":
        active_manga = manga.get_active_manga()
        return render_template(
            "auto_volume.html", active_manga=active_manga, len=len(active_manga)
        )
    else:
...
def manual_single():
    if request.method == "GET":
        ...
        if submit == "Main Menu":
            ...
            check = manga.check_manual_single_chapter(
                manga.SOURCE_PATH, manga_title, number
            )
            if check == False:
                session["error"] = manga.error_read()
                return redirect("/error")
            session["preview"] = manga.manual_single_chapter_preview(
                manga.SOURCE_PATH, manga_title, number, title
            )
            return redirect("/preview")
        else:
    ...
    if request.method == "GET":
        ...
        if submit == "Main Menu":
            ...
            if check == False:
                session["error"] = manga.error_read()
                return redirect("/error")
            session["preview"] = manga.manual_multiple_chapter_preview(
                manga.SOURCE_PATH, manga_title
            )
            return redirect("/preview")
        else:
...
def manual_volume():
    if request.method == "GET":
        ...
        if submit == "Main Menu":
            ...
            check = manga.check_manual_volume(
                manga.SOURCE_PATH, manga.DESTINATION_PATH, manga_title, number
            )
            if check == False:
                session["error"] = manga.error_read()
                return redirect("/error")
            session["preview"] = manga.manual_volume_preview(
                manga.SOURCE_PATH, manga_title, number, title
            )
            return redirect("/preview")
        else:
    ...
    if request.method == "GET":
        ...
        if submit == "Main Menu":
            ...
            if manga_format == "auto_chapter_batch":
                ...
                manga.manual_single_chapter_format(
                    manga.SOURCE_PATH,
                    manga.DESTINATION_PATH,
                    manga_title,
                    number,
                    title,
                )
                ...
                manga.manual_multiple_chapter_format(
                    manga.SOURCE_PATH, manga.DESTINATION_PATH, manga_title
                )
                return redirect("/manual")
            elif manga_format == "manual_volume":
                manga.manual_volume_format(
                    manga.SOURCE_PATH,
                    manga.DESTINATION_PATH,
                    manga_title,
                    number,
                ...
                if completed == True:
                    epub_src = os.path.join(
                        manga.MANGA_PAGE_PATH, manga_title, manga.VOLUME_SUBPATH
                    )
                    epub_dest = os.path.join(epub.MANGA_EPUB_PATH, manga_title)
                    epub.mass_generate_epub(epub_src, epub_dest, author, scans)
                else:
                    epub_src = os.path.join(
                        manga.MANGA_PAGE_PATH, manga_title, manga.VOLUME_SUBPATH, title
                    )
                    epub_dest = os.path.join(epub.MANGA_EPUB_PATH, manga_title)
        ...
        if submit == "Main Menu":
            ...
            if manga_format == "auto_chapter_batch":
                ...
                if check == False:
                    ...
                    return render_template("error.html", error=error)
                session.modified = True
                session["preview"] = manga.auto_chapter_batch_preview(
                    manga_title, title
                )
                return redirect("/preview")
            elif manga_format == "auto_volume":
                ...
                if check == False:
                    ...
                    return render_template("error.html", error=error)
                session.modified = True
                session["preview"] = manga.auto_volume_preview(
                    manga_title, number, title
                )
                return redirect("/preview")
            elif manga_format == "manual_single_chapter":
                check = manga.check_manual_single_chapter(
                ...
                check = manga.check_manual_multiple_chapter(
                    manga.SOURCE_PATH, manga_title
                )
                if check == False:
                    error = manga.error_read()
