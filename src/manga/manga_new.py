...
def add_new_manga(manga, writer, illustrator, completed):
    ...
    for volume in range(0, manga_src.count):
        ...
        volume_src = Directory(
            os.path.join(mc.SOURCE_PATH, manga_src.filenames[volume])
        )
        manga_format.manual_volume_format(
            volume_src.path,
            mc.DESTINATION_PATH,
            manga,
        ...
        volume_src.rm_dir()
    if completed == True:
        manga_chapters_pages = Directory(
            os.path.join(new_manga_directory, mc.CHAPTER_SUBPATH)
        )
...
def new_manga_check(manga, writer, illustrator):
    ...
    for volume in range(0, manga_src.count):
        ...
        volume_info = manga_src.filenames[volume].split(" - ")
        current_volume_number = int(volume_info[0])
        result = manga_check.check_manual_volume(
            volume_dir, mc.DESTINATION_PATH, manga, current_volume_number
        )
        if result == False:
            return False
    return True
...
def new_manga_sql_entry(manga, writer, illustrator, completed):
    ...
    for volume in range(0, manga_src.count):
        ...
        for chapter in range(0, current_volume.count):
            if current_volume.isfile(chapter):
                ...
                if len(chapter_info) == 2:
                    ...
                    current_chapter_title = ""
                msql.update_new_chapter(
                    manga, current_chapter_number, current_chapter_title
                )
        ...
        msql.update_new_volume(
            manga, current_volume_number, last_chapter_of_volume, current_volume_title
        )
