...
def check_manga_config():
    ...
    if (
        os.path.isdir(
            os.path.join(mc.MANGA_PAGE_PATH, "One Piece", mc.OP_COVER_SUBPATH)
        )
        == False
    ):
        me.error_write(
            f"[{os.path.join(mc.MANGA_PAGE_PATH,'One Piece',mc.OP_COVER_SUBPATH)}] NOT FOUND"
        )
        return False
    return True
        ...
        if source.isfile(chapter):
            ...
            for page in range(0, source_chapter.count):
                ...
                if source_chapter.ext(page) == -1:
                    me.error_write(
                        "INVALID SOURCE: ONE CHAPTER IS MISSING AN EXTENSION"
                    )
                    return False
    return True
