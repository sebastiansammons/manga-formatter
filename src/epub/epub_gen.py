...
def mass_generate_epub(src_path, dest_path, author, scans):
    ...
    for volume in volume_list:
        title = volume
        volume_src = os.path.join(src_path, title)
...
def generate_epub(src_path, dest_path, title, author, scans):
    ...
    temp_images = os.path.join(temp_path, ec.TEMP_IMAGES_SUBPATH)
    ...
    (
        ...
        toc,
        xhtml_pages,
    ) = build_template(temp_images, temp_path)
    build_xhtml_pages(page_id, xhtml_pages, max_height, max_width, temp_path)
    build_toc_xhtml(page_id, toc, temp_path)
    build_opf(
        opf_manifest_xhtml, opf_manifest_image, opf_spine, title, author, temp_path
    )
...
def copy_template(template_path, temp_path):
    try:
        ...
        shutil.chown(
            temp_path, user=int(os.getenv("PUID")), group=int(os.getenv("PGID"))
        )
...
def split_images(src_path, dest_path):
    ...
    for page in src_images:
        tmp_image = PIL.Image.open(os.path.join(src_path, page))
        ...
        if width > height:
            ...
            if len(tmp_filename) < 3:
                ...
                right_page_filename = os.path.join(
                    dest_path,
                    f"{tmp_filename[0]} - {tmp_filename[1][:9]}_a{tmp_filename[1][9:]}",
                )
                left_page_filename = os.path.join(
                    dest_path,
                    f"{tmp_filename[0]} - {tmp_filename[1][:9]}_b{tmp_filename[1][9:]}",
                )
            else:
                right_page_filename = os.path.join(
                    dest_path,
                    f"{tmp_filename[0]} - {tmp_filename[1]}_a - {tmp_filename[2]}",
                )
                left_page_filename = os.path.join(
                    dest_path,
                    f"{tmp_filename[0]} - {tmp_filename[1]}_b - {tmp_filename[2]}",
                )
            ...
            try:
                shutil.copy(os.path.join(src_path, page), os.path.join(dest_path, page))
                shutil.chown(
                    os.path.join(dest_path, page),
                    user=int(os.getenv("PUID")),
                    group=int(os.getenv("PGID")),
                )
            except PermissionError:
            ...
            if src_images[page].rfind("PG01") != -1:
                ...
                if src_images[page].rfind("PG01_a") != -1:
                    ...
                    if len(tmp_parse) < 3:
                        ...
                        blank_page_filename = (
                            f"{tmp_parse[0]} - {tmp_parse[1][:6]}00{tmp_parse[1][9:]}"
                        )
                    else:
                        # tmp_parse[1][:-2] + "00" replaces PG## with PG00
                        blank_page_filename = (
                            f"{tmp_parse[0]} - {tmp_parse[1][:-2]}00 - {tmp_parse[2]}"
                        )
                    # create blank page
                    # same dimensions as PG01
                    tmp_image = PIL.Image.open(os.path.join(src_path, page_filename))
                    width, height = tmp_image.size
                    tmp_image.close()
                    blank = PIL.Image.new(
                        mode="RGB", size=(width, height), color=(255, 255, 255)
                    )
                    blank.save(os.path.join(src_path, blank_page_filename))
                    blank.close()
            else:
...
def build_template(src_path, dest_path):
    ...
    for page in range(0, image_count):
        ...
        try:
            shutil.copy(
                os.path.join(src_path, src_images[page]),
                os.path.join(dest_path, "OEBPS/images", dest_image),
            )
            shutil.chown(
                os.path.join(dest_path, "OEBPS/images", dest_image),
                user=int(os.getenv("PUID")),
        ...
        tmp_image = PIL.Image.open(os.path.join(src_path, src_images[page]))
        width, height = tmp_image.size
        tmp_image.close()
        if page == 0:
            # Cover
            ...
            cover_manifest_image = cover_manifest_image.replace(
                "[CH###PG##]", page_id[page]
            )
            cover_manifest_image = cover_manifest_image.replace(
                "[img_filename]", dest_image
            )
            cover_manifest_image = cover_manifest_image.replace(
                "[img_ext]", get_mediatype(tmp_ext)
            )
            ...
            # Append Array for opf_manifest_image
            opf_manifest_image.append(
                generate_manifest_image(
                    page_id[page], dest_image, get_mediatype(tmp_ext)
                )
            )
...
def generate_toc(page_id, chapter_title):
    ...
    if "PG01" in page_id:
        ...
        if "PG01_b" in page_id:
            ...
            if chapter_title == "":
                toc = toc.replace(
                    "[chapter_title]", f"Chapter {str(chapter_num).lstrip('0')}"
                )
            else:
                toc = toc.replace(
                    "[chapter_title]",
                    f"Chapter {str(chapter_num).lstrip('0')}: {tmp_title}",
                )
            return toc
    else:
        return ""
...
def build_xhtml_pages(page_id, xhtml_pages, max_height, max_width, dest_path):
    for page in range(0, len(page_id)):
        ...
        xhtml_page_string = (
            xhtml_page_top + xhtml_pages[page] + template_xhtml_page.bottom
        )
        # xhtml title
        xhtml_page_string = xhtml_page_string.replace("[CH###PG##]", page_id[page])
        write_to_file(
            os.path.join(dest_path, "OEBPS/xhtml"),
            f"{page_id[page]}.xhtml",
            xhtml_page_string,
        )
...
def build_toc_xhtml(page_id, toc, dest_path):
    ...
    write_to_file(os.path.join(dest_path, "OEBPS"), "toc.xhtml", toc_xhtml)


def build_opf(
    opf_manifest_xhtml, opf_manifest_image, opf_spine, title, author, dest_path
):
    ...
    write_to_file(os.path.join(dest_path, "OEBPS"), "package.opf", opf)
...
def build_css(max_height, max_width, dest_path):
    ...
    write_to_file(os.path.join(dest_path, "OEBPS/css"), "stylesheet.css", css)


def build_epub(dest_path, title, temp_path):
    epub_filename = os.path.join(dest_path, f"{title}.epub")
    try:
        ...
        zip = ZipFile(epub_filename, "x")
    # zip mimetype
    zip.write(os.path.join(temp_path, "mimetype"), "mimetype")
    # zip container.xml
    zip.write(
        os.path.join(temp_path, "META-INF/container.xml"), "META-INF/container.xml"
    )
    # zip stylesheet.css
    zip.write(
        os.path.join(temp_path, "OEBPS/css/stylesheet.css"), "OEBPS/css/stylesheet.css"
    )
    # zip package.opf
    zip.write(os.path.join(temp_path, "OEBPS/package.opf"), "OEBPS/package.opf")
    # zip toc.xhtml
    zip.write(os.path.join(temp_path, "OEBPS/toc.xhtml"), "OEBPS/toc.xhtml")
    # zip images
    image_src = natsorted(
        [
            f
            for f in os.listdir(os.path.join(temp_path, "OEBPS/images"))
            if not f.startswith(".")
        ]
    )
    for i in range(0, len(image_src)):
        zip.write(
            os.path.join(temp_path, "OEBPS/images", image_src[i]),
            f"OEBPS/images/{image_src[i]}",
        )
    # zip xhtml
    xhtml_pages = natsorted(
        [
            f
            for f in os.listdir(os.path.join(temp_path, "OEBPS/xhtml"))
            if not f.startswith(".")
        ]
    )
    for i in range(0, len(xhtml_pages)):
        zip.write(
            os.path.join(temp_path, "OEBPS/xhtml", xhtml_pages[i]),
            f"OEBPS/xhtml/{xhtml_pages[i]}",
        )
    # chown
    try:
        shutil.chown(
            epub_filename, user=int(os.getenv("PUID")), group=int(os.getenv("PGID"))
        )
    except PermissionError:
        pass
...
def write_to_file(dest_path, filename, contents):
    file = open(os.path.join(dest_path, filename), "w")
    file.write(contents)
    file.close()
